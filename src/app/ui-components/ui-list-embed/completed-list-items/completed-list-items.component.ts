...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatButtonModule,MatFormFieldModule,MatInputModule,FormsModule,MatSelectModule,IndicatorCurrentlyFilteringComponent,MatIconModule],
...
ngOnInit(): void {
  ...
  this.listsService.getCompletedItems(this.listId).subscribe({
    next: (items) => {
      this.items = items;
      this.countChanged.emit(this.items.length); // ✅ Emit count here
      this.loading = false;
    },
    error: (err) => {
...
get filteredItems(): any[] {
  ...
  switch (this.selectedSortOption) {
    ...
    case 'title-desc':
      ...
      break;
    case 'date-asc':
      filtered = filtered.sort((a, b) => new Date(a.completedAt).getTime() - new Date(b.completedAt).getTime());
      break;
    case 'date-desc':
...
restoreItem(item: any) {
  this.listsService.uncompleteItem(item.id).subscribe({
    next: (restored) => {
      ...
      this.countChanged.emit(this.items.length); // ✅ Update count
    },
    error: (err) => {
