...
export class DialogEditSingleCategoryComponent implements OnInit {
  ...
  ngOnInit() {
    this.dashboardDropService.fetchCategories().subscribe((categories) => {
      const match = categories.find((cat: any) => cat.id === this.data.categoryId);
      if (match) {
        this.category = { ...match };
  ...
  save() {
    this.dashboardDropService.updateCategory(this.category.id, { name: this.category.name }).subscribe({
      next: () => {
