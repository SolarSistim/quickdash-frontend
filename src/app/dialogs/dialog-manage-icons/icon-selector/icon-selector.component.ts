...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MatTooltipModule, MatMenuModule, MatButtonModule, MatIconModule],
...
export class IconSelectorComponent {
  ...
  fetchIcons() {
    this.iconService.fetchIcons().subscribe(icons => {
      ...
      if (this.justUploadedFilename) {
        const uploadedIcon = this.icons.find(icon => icon.filename.toLowerCase() === this.justUploadedFilename?.toLowerCase());
        if (uploadedIcon) {
          this.selectedIcon = uploadedIcon.filename;
          this.iconSelected.emit(uploadedIcon.filename);  // ✅ Also emit it!
        }
        this.justUploadedFilename = null; // ✅ Clear it after selecting
      }
    });
  }
  ...
  openFileExplorer() {
    // ✅ Reset filter when starting upload
    this.filterText = '';
    ...
    fileInput.onchange = (event: any) => {
      ...
      if (file) {
        ...
        this.uploadInProgress.emit(true); // ✅ signal upload started
      }
    };
    fileInput.click();
  }
  ...
  checkFilenameExists(filename: string) {
    this.filenameExists = this.icons.some(icon => icon.filename.toLowerCase() === filename.toLowerCase());
  ...
  uploadIcon() {
    ...
    const filenameToSelect = this.selectedFile.name; // ✅ Save the uploaded file's name
  
    this.iconService.uploadIcon(this.selectedFile, this.title, this.description).subscribe({
      next: () => {
        this.justUploadedFilename = filenameToSelect;
        this.resetUpload();
        this.fetchIcons();
      
        this.uploadInProgress.emit(false); // ✅ signal upload finished
      },
      error: (err) => {
  ...
  resetUpload() {
    ...
    this.uploadInProgress.emit(false); // ✅ signal upload canceled
  }
